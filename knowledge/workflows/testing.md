...
- Job lifecycle storage/actions: `tests/jobState.test.ts` validates backward-compatible lifecycle defaults for legacy jobs, preservation of existing lifecycle state, idempotent normalization, and completed legacy job mapping; `tests/useJobsLifecycle.test.ts` validates all public `useJobs` lifecycle actions, invalid-transition blocking, persistence, multi-visit history, and reopening with original completion timestamp preserved.
...
**Last Updated:** 2026-08-15 (Job lifecycle hook action tests)
