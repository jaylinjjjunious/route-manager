...
- **Field job creation + technician workflow polish:** `JobModal` now offers device-type multi-select for `field_task` jobs and shows a matching procedure suggestion. `JobDetailModal` renders device badges, adds a `Continue Procedure` button that scrolls to the next unresolved step, and improves closeout blocker navigation with jump-to-step links. `ProcedureWorkspace` adds stable step scroll IDs, highlights the current step, quiets satisfied steps, and enlarges blocking-action tap targets for phone use.
...
**Last Updated:** 2026-08-16 (Test suite: 430/430 passing; added field job creation + technician workflow polish for Sonic procedure: device selection, Continue Procedure, scroll-to-step, mobile ergonomics)
