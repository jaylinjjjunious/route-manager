...
- Mission Control/Today dashboard uses the AIØ week strip inside the top weather card, the expanded day panel (`ExpandedDayPanel`), and move-to-day sheet (`MoveToDaySheet`).
...
- **AIØ three-tab navigation (Today / Jobs / More):** iOS-style redesign of the primary interface. The former Mission Control dashboard is now the simplified **Today** screen: Weather / dimensional artwork, This Week calendar, Next Best Jobs / Current Job, and Today's Other Jobs. The Travel Plan panel and inline Preview/Ride Mode readiness content are no longer rendered on the main dashboard. The compact dark weather panel shows live current weather top-left (sun/moon glyph or dimensional artwork, temperature, condition, feels-like) from Open-Meteo with permission-respecting location and offline/denied fallbacks; live weather is display-only. **Jobs** is the schedule list (today, later days, Route B standby, overdue/unscheduled review), and **More** is the hub for all legacy tabs (Inventory, Battery, Tracker, Habits, Tools, Settings) plus Proof Vault, Add Process Serve, Import Screenshots, Debug Center, account, theme, and Sign Out. New AIØ design tokens (`--color-aio-*`) and primitives in `src/components/aio/`. Dev-only screenshot mode (`VITE_TODAY_SCREENSHOT_MODE`) and `scripts/screenshot-today.mjs` for headless layout verification (320/390/430 px, dark/light).
- **Dynamic Weather Artwork:** Realistic, dimensional 3D weather artwork on the Today screen first card. Normalizes real Open-Meteo weather parameters into 21 semantic states (sunny, mostly-sunny, partly-cloudy, hazy-sun, very-hot, cloudy, overcast, light-rain, heavy-rain, thunderstorm, snow, heavy-snow, sleet, fog, windy, clear-night, partly-cloudy-night, cloudy-night, sunrise, sunset, fallback) with an asset registry mapping to `/weather/*.png`. Includes 5-minute memory cache to avoid repeat fetches on tab change, 15-minute periodic live refresh, and graceful fallback to Lucide glyphs if assets are missing or fail to load.
...
- **Official app icon:** Preserved source artwork and generated black/white AIØ favicon/PWA icon assets (`public/icons/aio-icon-192.png`, `public/icons/aio-icon-512.png`). The header brand mark is the `AIØ 17` text wordmark, with a red/white/blue blended `17`, not a logo image.
...
- **Apple iOS Wrap (GitHub Actions):** Native Apple iOS wrapping via Capacitor (`@capacitor/core`, `@capacitor/ios`, `capacitor.config.ts`, `ios/` Xcode workspace). CI/CD workflow `.github/workflows/apple-wrap.yml` runs on `macos-latest`, compiles Vite build output, syncs iOS assets, builds an Xcode archive (`App.xcarchive`), and attaches the zipped archive artifact to GitHub Actions runs.
- **Sonic Verifone Device Swap procedure (`sonic-verifone-device-swap` v1.0.0):** First real customer procedure implemented entirely as data on the generic engine. Covers V400M, UX301, and UX401 device workflows with conditional device-type activation, pre-removal proof requirements, serial capture (`old`/`new`/`old_and_new`), grounding-strap warnings, testing checkpoints, return obligations, and troubleshooting reference content. Registered in the default procedure catalog alongside the generic fixture. Jobs gain optional `deviceType?: string` and `deviceTypes?: string[]` fields to drive procedure conditional logic. A dev-only Sonic procedure harness creates 6 pre-assigned test jobs (V400M, UX301, UX401, and combinations) behind `VITE_ENABLE_SONIC_PROCEDURE_HARNESS=true`.
- **Job Detail tab-based redesign (WORK / PROCEDURE / CLOSEOUT / DETAILS):** `JobDetailModal` replaces the giant continuous-scroll experience with a sticky segmented tab bar. The WORK tab focuses on job identity, Next Action, warnings, and a compact procedure-progress tile. The PROCEDURE tab hosts the full `ProcedureWorkspace` with a phase accordion (current phase expanded, completed/future phases collapsed) and progressive step disclosure (current step and blocking steps expanded, satisfied steps collapsed). The CLOSEOUT tab isolates the closeout gate with a compact summary and tappable blocker rows that jump to the correct procedure step. The DETAILS tab holds schedule, pay, time summary, visit history, admin controls, and legacy status UI. A sticky bottom action bar shows the primary lifecycle action and Continue Procedure when appropriate. Typography was increased across both files: no meaningful content below 12px, with 18–20px titles, 15–17px step titles, and 14–16px body/button text. Quick Mode now hides verbose content and satisfied requirements behind "Show full instructions" links. Guided Mode renders instructions as readable paragraphs with auto-expanded current and blocking steps only. All existing lifecycle, proof, inventory, closeout, and procedure semantics are preserved; tabs are presentation-only.
