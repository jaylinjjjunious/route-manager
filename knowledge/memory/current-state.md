...
- **Sonic Verifone Device Swap procedure (`sonic-verifone-device-swap` v1.0.0):** First real customer procedure implemented entirely as data on the generic engine. Covers V400M, UX301, and UX401 device workflows with conditional device-type activation, pre-removal proof requirements, serial capture (`old`/`new`/`old_and_new`), grounding-strap warnings, testing checkpoints, return obligations, and troubleshooting reference content. Registered in the default procedure catalog alongside the generic fixture. Jobs gain optional `deviceType?: string` and `deviceTypes?: string[]` fields to drive procedure conditional logic.
...
**Last Updated:** 2026-08-16 (Added Sonic Verifone Device Swap procedure v1.0.0 as first real customer data-driven procedure)
