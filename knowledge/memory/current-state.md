...
- **Job system:** 5 types, legacy statuses, completion workflow, proof vault, and optional v1 lifecycle state normalized at load/storage while `JobStatus` remains authoritative. `useJobs` now exposes persisted lifecycle transition actions for check-in, work state changes, visit ending, closeout completion, and reopen; no UI is wired to them yet.
...
**Last Updated:** 2026-08-15 (Job lifecycle hook actions exposed without UI wiring)
