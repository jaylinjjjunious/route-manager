...
- **Job system:** 5 types, legacy statuses, completion workflow, proof vault, and optional v1 lifecycle state normalized at load/storage while `JobStatus` remains authoritative. `useJobs` exposes persisted lifecycle transition actions for check-in, work state changes, visit ending, closeout completion, and reopen. `JobDetailModal` opens with a lifecycle-aware Job Overview and derived Next Action card; check-in, ready/start, pause/resume, awaiting-support, blocked-onsite, and end-visit actions are wired with mobile note sheets where needed. Compact Visit History shows visit numbering, timestamps, end reasons, and stable visit IDs for later proof linking. Closeout Gate remains deferred.
...
**Last Updated:** 2026-08-15 (Job Detail multi-visit history added; Closeout Gate deferred)
