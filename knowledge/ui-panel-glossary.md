...
- **Current review status:** The current Road Readiness visual, live-weather, Preview Guide gating, five-section review flow, completion feedback, reduced-motion behavior, and responsive layout are implemented and approved in production. A responsive weather-row bug fix (2026-08-05) corrected the live-weather header so it never wraps one character at a time: `Feels like XX°` is nowrap, the condition uses word wrapping only, the divider stays short and vertically centered, and the compact panel height is restored at 320/390/430px. A follow-up compact header-actions refinement (2026-08-05) shrank the status pill and the Preview Guide primary button, right-aligned the stacked controls on narrow phones, and restored side-by-side weather-left / controls-right at `min-[410px]` with a `flex-wrap` safety fallback. The panel is considered functionally complete for the current phase, but not permanently closed because the planned route-aware battery readiness feature remains intentionally deferred for a later phase.
- **Locked visual direction:** Compact dark black-and-purple card (removed the bright-blue full-card treatment). Live weather sits in the top-left as a single horizontal weather row — a polished sun glyph by day (moon glyph at night) beside the large temperature, a slim vertical divider (short, vertically centered, kept tight to the temperature), and a right two-line stack with the feels-like temperature on top and condition text below. `Feels like XX°` is `whitespace-nowrap` and never wraps; the condition uses normal word wrapping only (never `break-words`), so no line ever wraps one character at a time. Below the `min-[410px]` breakpoint (covers the 320/390/430px phone widths) the weather row takes the full header width and the readiness actions stack below it as a compact right-aligned group; at `min-[410px]` and up the weather row sits top-left at its natural width with the status pill and primary action on the top-right. This replaced the earlier `min-[390px]` side-by-side layout, which starved the weather row of width at 390/430px and caused character-by-character wrapping and an over-tall panel. The header action column is `flex-wrap` with the weather row `shrink-0` and the actions pushed right via `ml-auto`, so an unusually wide disabled label (`Preview Guide Unavailable`, ≈192px) wraps below the weather as a right-aligned group at 410–430px instead of squeezing the nowrap feels-like text (the weather row needs ≈186px minimum). The status pill is compact (`px-2 py-0.5 text-[11px]`, 12px icon, `gap-1`, one-line uppercase label), and the primary action is a compact white pill (`min-h-11` = 44px touch target, `px-2 text-[12px]`, 14px icon, icon + label on one line, never full-width, right-aligned). Readiness status pill stays in the top-right above the compact white pill primary action. Readiness message, wind chip, and informational battery chip follow below; the Preview Guide and battery checklist rows remain informational. Weather is display-only and never gates Road Readiness/Ride Mode (the manual wind setting is the only weather gate). When live weather is loading or unavailable (offline/denied permission), the glyph fades and the slot shows `…`/`Live weather unavailable` without fabricating values.
