...
- **Primary AIØ brand logo locator:** in `AioHeader.tsx`, the `<h1 className="mt-0.5 leading-none">` in the left `min-w-0` block (directly below the greeting, above the date line). It contains a single `<img src="/branding/aio-logo-black.svg" alt="AIØ logo" className="block h-9 w-9 object-contain">`. The logo position (`mt-0.5`, `items-end` bottom alignment, `gap-3` spacing from the profile button) is preserved from the former text wordmark.
- **Brand asset path:** `public/branding/aio-logo-black.svg` (512×512; black rounded-rect badge with white `AIØ` lettering), referenced in code as `/branding/aio-logo-black.svg` via `const LOGO_PATH` in `src/components/aio/AioHeader.tsx`. The legacy truncated `public/branding/aio-logo-black.png` is NOT used.
- **Locked brand direction:** The primary AIØ brand mark is the bundled black-background / white-lettering `aio-logo-black.svg` badge. It is rendered as-is with `object-contain` — never cropped, stretched, recolored, redrawn, or overlaid with text. It reads identically in dark and light mode (no theme-dependent recoloring).
