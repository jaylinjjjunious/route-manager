...
- **Location in UI:** The sticky top bar of the app, rendered above the main content on the Today (`dashboard`), Jobs, and More tabs. It contains the greeting, the `AIØ 17` wordmark, the current date, and the profile-picture button on the right.
- **Purpose:** Global app header that identifies the app (`AIØ 17`), greets the signed-in user, shows the current date, and provides one-tap access to the profile / More destination.
...
- **Primary AIØ brand logo locator:** in `AioHeader.tsx`, the `<h1 className="mt-0.5 text-[34px] font-black leading-none tracking-[-0.02em] text-[var(--color-aio-text)]">` in the left `min-w-0` block (directly below the greeting, above the date line). It renders `AIØ` in the theme text color followed immediately by `17` in the same inherited typography. The `17` uses a soft vertical red-at-top, white-at-center, blue-at-bottom blend. The complete heading is exposed to assistive technology as `AIØ 17`. The logo position (`mt-0.5`, `items-end` bottom alignment, `gap-3` spacing from the profile button) is preserved.
- **Two distinct brand marks (never mix):** (1) **In-app header brand mark** = the `AIØ 17` text wordmark in `AioHeader.tsx`. (2) **Home Screen/PWA app icon** = the black-background, white-`AIØ` badge artwork (`public/icons/aio-icon-192.png`, `public/icons/aio-icon-512.png`, `public/apple-touch-icon.png`, `public/favicon-*.png`). The header never renders the badge image, and the badge artwork is never rendered in the header.
...
- **Locked brand direction:** The in-app header brand mark is the text wordmark `AIØ 17`, rendered at `text-[34px] font-black leading-none tracking-[-0.02em]`. `AIØ` keeps the theme text color (`--color-aio-text`); only `17` receives the Firecracker-inspired red/white/blue fade. The bundled badge images `public/branding/aio-logo-black.svg` / `aio-logo-black.png` are NOT used in the header and must not be re-added. The black/white AIØ badge artwork is used only for favicons and PWA icons (`public/icons/aio-icon-192.png`, `public/icons/aio-icon-512.png`).
...
- **Current review status:** The current Road Readiness visual, live-weather, Preview Guide gating, five-section review flow, completion feedback, reduced-motion behavior, and responsive layout are implemented and approved in production. A responsive weather-row bug fix (2026-08-05) corrected the live-weather header so it never wraps one character at a time: `Feels like XX°` is nowrap, the condition uses word wrapping only, the divider stays short and vertically centered, and the compact panel height is restored at 320/390/430px. A follow-up compact header-actions refinement (2026-08-05) shrank the status pill and the Preview Guide primary button and finalized the responsive rule: the tested 390px and 430px widths are a strict two-column header (weather left at natural width, controls top-right) and never stack; only 320px (any width below the `min-[381px]` breakpoint) stacks the controls right-aligned below the weather, with a `flex-wrap` safety fallback. The panel is considered functionally complete for the current phase, but not permanently closed because the planned route-aware battery readiness feature remains intentionally deferred for a later phase.
- **Locked visual direction:** Compact dark black-and-purple card (removed the bright-blue full-card treatment). Live weather sits in the top-left as a single horizontal weather row — a polished sun glyph by day (moon glyph at night) beside the large temperature, a slim vertical divider (short, vertically centered, kept tight to the temperature), and a right two-line stack with the feels-like temperature on top and condition text below. `Feels like XX°` is `whitespace-nowrap` and never wraps; the condition uses normal word wrapping only (never `break-words`), so no line ever wraps one character at a time. Below the `min-[381px]` breakpoint (covers 320px and any narrower phone) the header stacks: the weather row takes the full header width and the readiness actions sit right-aligned below it. At `min-[381px]` and up the header is a strict two-column row — weather left at its natural width (≈171px), status pill and primary action top-right — so the tested 390px and 430px phones are always side-by-side (390px: gap ≈18px; 430px: gap ≈58px) with no stacking, clipping, or overflow. Side-by-side needs ≈324px of content width (weather ≈171px + 8px gap + actions ≈145px), which after the panel's 28px side padding is a ≈381px viewport; below that the two columns cannot fit without clipping or shrinking the locked weather typography, so they stack instead. The weather row stays horizontal at every width: `Feels like XX°` is `whitespace-nowrap` and never wraps, and the condition uses normal word wrapping only (never `break-words`). The container is `flex-wrap` with the weather row `shrink-0` and the actions pushed right via `ml-auto`, so an unusually wide disabled label (`Preview Guide Unavailable`, ≈192px) wraps below the weather as a right-aligned group instead of squeezing the nowrap feels-like text. The status pill is compact (`px-1.5 py-0.5 text-[10px]`, 11px icon, `gap-1`, one-line uppercase label), and the primary action is a compact white pill (`min-h-11` = 44px touch target, `px-1 text-[11px]`, 12px icon, icon + label on one line, never full-width, right-aligned). Readiness status pill stays in the top-right above the compact white pill primary action. Readiness message, wind chip, and informational battery chip follow below; the Preview Guide and battery checklist rows remain informational. Weather is display-only and never gates Road Readiness/Ride Mode (the manual wind setting is the only weather gate). When live weather is loading or unavailable (offline/denied permission), the glyph fades and the slot shows `…`/`Live weather unavailable` without fabricating values.
...
- **Primary code file:** `src/features/jobs/JobDetailModal.tsx`
