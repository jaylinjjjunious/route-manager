...
- **Profile-picture button:** a single `<button>` in the right-hand `flex shrink-0 items-center gap-2` container, `h-11 w-11 overflow-hidden rounded-full` with the local `img src="/profile/avatar.webp"` rendered `h-full w-full object-cover` (circular, cropped), `aria-label="Open profile and More menu"`, `title="Profile"`. Alt text: `Your profile picture`.
...
- **Strict CLI edit boundary:** Edit only `src/components/aio/AioHeader.tsx` plus the single `AioHeader` usage in `src/App.tsx` (`{/* Header */}`). Do not modify the greeting/logo/date block, the `AIØ` brand, Today screen panels, the bottom navigation, `MoreScreen`, global theme logic, or the non-AIØ `Header` component used on other tabs.
- **Locked rule:** The AIØ Top Header uses the profile picture as its single right-side control instead of separate More (three-dots) and light/dark theme buttons. The three-dots button and the header theme button are removed and must not be re-added to this header. Dark/light mode remains available in the app through `MoreScreen` (Account row theme toggle) and Settings; global theme logic in `src/App.tsx` (`theme`, `handleToggleTheme`) is unchanged.
