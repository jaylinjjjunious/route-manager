...
The minimal condition evaluator in `src/features/jobs/procedures/procedureConditions.ts` supports `device_type_equals`, `job_field_equals`, `previous_answer_equals`, `equipment_present`, `equipment_missing`, `issue_present`, and `blocker_present`. The `device_type_equals` condition now supports both a single `deviceType` string and a `deviceTypes` array for jobs that involve multiple devices. Missing context returns unresolved/inactive rather than active. If a job has a procedure assignment that cannot be resolved or is malformed, effective closeout adds a synthetic unsatisfied required requirement and blocks final completion because procedure-dependent obligations cannot be verified safely.

### Procedure Workspace UI

`ProcedureWorkspace` lives inside the **PROCEDURE** tab. It shows the assigned procedure name/version, a Guided/Quick presentation toggle, compact progress summary (completed/applicable steps, missing items, next step title), and a phase accordion. Each phase header displays the phase name, completion count, and missing-item count, and is tap-to-expand/collapse. Default expansion logic: the phase containing the current step is expanded; completed phases are collapsed; future phases are collapsed; inactive conditional phases are collapsed.

**Step cards** use progressive disclosure. A collapsed step shows only the step number or completion indicator, title, status badge (Current / Missing / Complete / Optional), and a critical-warning icon when applicable. An expanded step reveals Guided instructions (broken into readable paragraphs), the Quick checkpoint, proof requirements, inventory/serial inputs, testing actions, support information, and warning details. In **Quick Mode**, satisfied requirements are hidden, long explanations are collapsed behind a "Show full instructions" link, and only the step title, quick checkpoint, critical warning, and blocking proof/inventory/test actions remain visible. In **Guided Mode**, instructions are rendered as readable paragraphs/bullets with whitespace, and only the current step and steps with blocking requirements are auto-expanded.

The first local catalog fixture is customer-agnostic (`generic-field-work` v1.0.0). The first real customer procedure is `sonic-verifone-device-swap` v1.0.0 (`Sonic Verifone Device Swap`), covering V400M terminal replacement and UX301/UX401 display swap with pre-removal documentation, grounding-strap warnings, serial capture, testing, and return obligations. It is implemented entirely as procedure data on the generic engine with no Sonic-specific React components or lifecycle logic.
...
**Technician workflow polish:** `JobDetailModal` renders selected device badges in the job header. When a technician is actively working an in-progress job, the sticky bottom action bar shows a `Continue Procedure` button that switches to the **PROCEDURE** tab and opens the next unresolved step (phase expanded, step expanded). The closeout blocker summary in the **CLOSEOUT** tab adds a prominent jump-to-step button when missing required items exist, linking directly to the next incomplete procedure step.

**Mobile ergonomics in `ProcedureWorkspace`:** Every step renders a stable DOM `id="step-{stepId}"` for deep-link scrolling. The current (next unresolved) step is highlighted with a blue border and a "Current" badge. Satisfied steps are visually quieted with reduced border/bg opacity. Blocking/missing steps retain a rose-tinted container. Proof capture, inventory record, and test-acknowledge buttons use full-width, high-contrast styling when the requirement is blocking, with `min-h-10` touch targets. All meaningful text is at least 12px; step titles are 15–17px, body text 14–16px, and metadata 12–13px.
