...
- **Free tier:** 1,500 req/month, 5 req/min. The server enforces a sliding-window 5/min rate limiter, queues over-limit requests (bounded, 45s timeout), and serves stale cache data while rate-limited (stale-while-revalidate). A durable monthly budget store (1,500/month, persisted to `.local-transit-usage/usage.json`) hard-gates upstream calls near the monthly ceiling.
...
- Components in `src/components/transit/`:
  ...
  - `TransitStatusCard` — Settings diagnostic (configured flag, rate limit, monthly budget, cache, last error, clear-cache button).
...
Browser (VITE_TRANSIT_PROVIDER=transit)
  ...
  → server/transit/transitService (cache → rate limiter → queue → monthly budget)
...
4. The monthly budget guard runs before any upstream call: nonessential categories (`alerts`, `networks`) are blocked past 85%, everything but `plan`/`arrivals` past 95%, everything past 1,500. Blocked requests serve stale cache if present and otherwise return 429 `TRANSIT_MONTHLY_BUDGET_EXHAUSTED`. Only requests that reach the upstream count.
...
- **Monthly budget exhausted/reserved** → no upstream call is made (no limiter slot spent); stale cache served when available, otherwise 429 `TRANSIT_MONTHLY_BUDGET_EXHAUSTED`.
...
- `server/transit/` — router, service, client, cache, limiter, budget, types
